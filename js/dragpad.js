...
function dragpad(parentElement, options) {
    ...
    switch (event.type) {
      case "mousedown":
        ...
        if (props.handle.capture) {
          if (callOnStartMove() === false) {
            props.handle.capture = false;
          } else {
            document.__dpCapture = props.handle;
          }
        }
        break;
      case "mouseup":
  ...
  function DOMInsert() {
    ...
    if (!document.__dpListener) {
      document.__dpListener = true;
      document.__dpCapture = null;
      document.addEventListener("mouseup", function() {
        if (document.__dpCapture !== null) {
          document.__dpCapture.capture = false;
          document.__dpCapture = null;
        }
      });
      document.addEventListener("mousemove", function(docEvnt) {
        if (document.__dpCapture !== null) {
          document.__dpCapture.element.dispatchEvent(new MouseEvent(docEvnt.type, docEvnt));
        }
      });
    }
  ...
  if (typeof self === "object" && self !== null) {
    ...
    this.wrapperAddClass = function(cls) { props.wrapper.element.classList.add(cls); };
    this.padAddClass     = function(cls) { props.pad.element.classList.add(cls); };
    this.handleAddClass  = function(cls) { props.handle.element.classList.add(cls); };

    this.wrapperRemoveClass = function(cls) { props.wrapper.element.classList.remove(cls); };
    this.padRemoveClass     = function(cls) { props.pad.element.classList.remove(cls); };
    this.handleRemoveClass  = function(cls) { props.handle.element.classList.remove(cls); };
